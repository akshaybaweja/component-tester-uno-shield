...
/* USART0 */
  ...
  #define ISR_USART_RX   USART_RX_vect      /* ISR */
#endif
...
ISR(ISR_PINCHANGE, ISR_BLOCK)
{
  ...
  if (! (SERIAL_PIN & (1 << SERIAL_RX)))     /* low state */
  {
     ...
     *  - 1/2 bit period of 9600bps = 52.08�s
     *  - fixed prescaler = 8
     *    MCU clock [MHz]:    8    16    20
     *    timer cycle [�s]:   1    0.5   0.4
     *    cycles for 52�s:    52   104   130
     *  - we don't need high precision, just good enough to sample 10 bits
     *  - MCU_CYCLE_TIME in 0.1ns
     *    timer cycles = (52�s * 10000) / (MCU_CYCLE_TIME * 8)
     *  - use delay of 48�s for early sampling and to allow some time jittering
     */
...
ISR(TIMER0_COMPB_vect, ISR_BLOCK)
{
  ...
  else                   /* more bits expected */
  {
     ...
     *  - bit period of 9600bps = 104.1�s
     *  - fixed prescaler = 8 (<=16MHz) or 64 (>16MHz)
     *    MCU clock [MHz]:    8    16    20
     *    timer cycle [�s]:   1    0.5   3.2
     *    cycles for 104�s:   104  208   32.5
     *  - we don't need high precision, just good enough to sample 10 bits
     *  - MCU_CYCLE_TIME in 0.1ns
     *    timer cycles = (104�s * 10000) / (MCU_CYCLE_TIME * prescaler)
     *  - use a delay of 102�s to compensate for the runtime of this ISR
     */
...
void Serial_Setup(void)
{
  ...
  // REG_UBRR = (CPU_FREQ / (16UL * 115200)) - 1;
  REG_UBRR = 103; //9600bps @ 16MHz
